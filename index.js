...
function getFullNames(runners) {
  let newArray = [];
  runners.forEach((items) => newArray.push(`${items.last_name}, ${items.first_name}`));
  return newArray;
}
...
function firstNamesAllCaps(runners) {
  return runners.map((items) => items.first_name.toUpperCase());
...
function tallyUpDonations(runners) {
  let donations = runners.map((items) => items.donation);
